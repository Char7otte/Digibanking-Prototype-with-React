...
function Transaction() {
  ...
  useEffect(() => {
    ...
    articles.forEach((article) => article.addEventListener("click", selectCardArticle));
            ...
            <section>
              {transactionStep === 0 && (
                <>
                  ...
                  {accounts.current.map((account) => (
                    <AccountCardComponent accountData={account} key={account.number} />
                  ))}
                </>
              )}
              ...
              {transactionStep === 1 && (
                <>
                  ...
                  <label htmlFor="transferee-account-number" className={styles.label}>Account number</label>
                  <input type="text" name="transferee-account-number" id="transferee-account-number" required placeholder="Account number" onChange={updateTransfereeAccount} className="spacing-md" />
                </>
              )}
              ...
              {transactionStep === 2 && (
                <>
                  ...
                  <input type="number" required min={0} name="transferee-account-number" id="transferee-account-number" placeholder="Transfer Amount" onChange={updateTransferAmount} className="spacing-md" />
                </>
              )}

              <div className="d-flex justify-content-between">
                <button className="m-2 ms-0" onClick={returnToDashboard}>Return</button>
                {transactionStep < 2 && (
                  <button className="important-button m-2 me-0" onClick={moveTransactionStep}>Continue</button>
                )}
                {transactionStep === 2 && (
                  <button className="important-button m-2" onClick={executeTransfer}>Transfer</button>
                )}
              </div>
            </section>
          </main>
    ...
    <>
      <div className="transfer-page">
        ...
        <div className="transfer-tabs">
          <button className="tab spacing-md active" onClick={() => handleModeChange("local")} id="localButton">Local Transfer</button>
          <button className="tab spacing-md" onClick={() => handleModeChange("overseas")} id="overseasButton">Overseas Transfer</button>
          ...
          <form>
            ...
            <select className="input-select spacing-md" required>
              <option value={user.account_number}>{user.account_type} — **** {String(user.account_number).slice(-4)} ({user.currency})</option>
            </select>
            ...
            <input type="text" name="remarks" className="input spacing-md" placeholder="Add a note" />

            <div className="transfer-buttons">
              <button type="submit" className="button red full">Transfer Now</button>
              <button type="button" className="button outline full">Schedule Transfer</button>
            </div>
          </form>
        </div>
      </div>

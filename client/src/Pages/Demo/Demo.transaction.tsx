...
function DemoTransaction() {
    ...
    async function executeTransfer() {
        try {
            const response = await api.post("/api/transfer?mode=demo", {
                to: selectedTransfereeAccount.current,
                amount: transferAmount.current,
                reference: "Demo Transfer",
            });
            alert(
                `Transfer successful! New balance: $${response.data.balance}`,
            );
            navigate("/demo");
        } catch (error: any) {
            alert(
    ...
    if (isLoading)
        return <div className="loadingContainer">Loading demo...</div>;
    if (!demoData)
        return <div className="loadingContainer">Failed to load demo data</div>;
                ...
                <main className={styles.mainContainer}>
                    ...
                    <section>
                        {transactionStep == 0 && (
                            <>
                                <h2 className="mb-4">
                                    Step 1: What account do you want to transfer
                                    from?
                                </h2>

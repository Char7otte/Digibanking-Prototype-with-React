...
export async function useEyeClick({ dwellMs = 800, enabled = true }: UseEyeClickOpts = {}) {
  ...
  useEffect(() => {
    ...
    const smoothPrediction = async (rawPrediction: {x: number, y: number}) => {
      ...
      predictionHistoryRef.current = predictionHistoryRef.current.filter(pred => {
      ...
      try {
          ...
          try {
            ...
            if (attempt === 1) {
              try {
                ...
                console.log('CPU backend unavailable, proceeding with default backend')
              }
            }
            ...
            for (let i = 0; i < 10; i++) {
              try {
                ...
                if (testPred && typeof testPred.x === 'number' && typeof testPred.y === 'number') {
                  console.log('Got valid prediction:', testPred)
                  hasValidPredictions = true
                  break
                }
              } catch (predErr) {
                console.warn('Prediction test error:', predErr)
              }
              await new Promise(resolve => setTimeout(resolve, 500))
            }
            
            if (!hasValidPredictions) {
              throw new Error('No valid predictions after initialization')
            }
        ...
        try {
          ...
          if (calibData) {
            ...
            offsetRef.current = { x: parsed.offsetX || 0, y: parsed.offsetY || 0 }
        ...
        window.addEventListener('eyeCalibrationUpdated', handleCalibrationUpdate)
          ...
          try {
            ...
            if (prediction && typeof prediction.x === 'number' && typeof prediction.y === 'number') {
              ...
              smoothPrediction(prediction).then((smoothedPrediction) => {
                ...
                const correctedX = smoothedPrediction.x + offsetRef.current.x - window.scrollX
                const correctedY = smoothedPrediction.y + offsetRef.current.y - window.scrollY
                ...
                const elements = document.elementsFromPoint(correctedX, correctedY)
                const clickableEl = elements.find(el => 
            ...
            if (consecutiveErrors <= 3) {
              console.warn(`Prediction error ${consecutiveErrors}:`, error.message)
            }
            
            // If too many consecutive errors, restart WebGazer
            if (consecutiveErrors >= maxConsecutiveErrors) {
              console.error('Too many consecutive errors, restarting WebGazer...')
              try {
                webgazer?.pause?.()
                setTimeout(async () => {
                  try {
        ...
        return () => {
          running = false
          window.removeEventListener('eyeCalibrationUpdated', handleCalibrationUpdate)
        }
    ...
    return () => {
      ...
      try {
        webgazer?.pause?.()
        const dot = document.getElementById('eye-prediction-dot')
        dot?.remove()
      } catch {}
    }
  }, [dwellMs, enabled])

...
export function Calibration({ onComplete }: { onComplete?: () => void }) {
  ...
  const collectedDataRef = useRef<Array<{ screen: { x: number, y: number }, prediction: { x: number, y: number } }>>([])
  ...
  const handlePointClick = async (point: CalibrationPoint) => {
    ...
    if (nextPoint >= calibrationPoints.length) {
      ...
      localStorage.setItem('eye_calibration', JSON.stringify({
        offsetX: offset.x,
        offsetY: offset.y,
        timestamp: Date.now()
      }))
    ...
    return (
      <div className={styles.calibrationContainer}>
        <div className={styles.calibrationCard}>
          ...
          <p>Calibration improves the accuracy of eye tracking by learning where you look on the screen.</p>
  ...
  return (
    <div className={styles.calibrationOverlay}>
      ...
      <div className={styles.instruction}>
        Look at the orange circle and click it ({currentPoint + 1}/{calibrationPoints.length})
      </div>
      ...
      {calibrationPoints.map((point) => (
        ...
        >
          {point.id < currentPoint && <span className={styles.checkmark}>✓</span>}
        </div>
      ))}
    </div>
  )
